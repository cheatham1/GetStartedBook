...
The signal process in Histogram Analyser is the Higgs boson decaying into two $$W$$ bosons which subsequently decay into leptons  \(ℓ\) and neutrinos \($$\nu$$\):
...
Often physicists refer to a particle and its [antiparticle](https://en.wikipedia.org/wiki/Antiparticle) collectively by just the particle's name.  So, when we talk about quarks we mean quarks and antiquarks.  Similarly for neutrinos and antineutrinos.  
